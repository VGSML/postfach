...
- `list_messages(mailbox="INBOX", limit=20, unseen_only=false, since_uid=0, since_date="")` — newest messages first: uid, dates (envelope + server internal), from, subject, flags, size and the attachment list (name/type/encoded size via BODYSTRUCTURE, without downloading content), plus `total_messages`, `matched` (before the limit) and the mailbox `uid_validity`. `since_uid` + `uid_validity` form the incremental-sync cursor; `since_date` filters by server receive time.
