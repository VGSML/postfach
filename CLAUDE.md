...
- **Email content is untrusted input.** Every string originating from a mailbox (subjects, bodies, sender names, attachment filenames) passes through the screening layer before it is placed in a tool result. Screening is layered: cheap heuristics, the language allowlist gate (`POSTFACH_ALLOWED_LANGS`, default en,de,fr,it,es,ru — text the stack cannot vet is flagged; script check catches embedded foreign-script fragments; statistical language ID runs only on prose-like text, structured text derails it), then the Prompt Guard 2 classifier. This layer sits between the mail client and the tool response and is not optional per-tool.
...
- **ChatGPT/Codex plugin**: `.codex-plugin/plugin.json` (same `skills/` dir; `mcpServers` points at gitignored `.codex-plugin/mcp.json` that `install.sh` generates with absolute paths + env). Installed in developer mode via a local marketplace — ChatGPT runs local stdio MCP servers directly; see `~/projects/hugr-lab/skills/hugr-plugin` for the working reference. `codex mcp add` is the CLI alternative.
- Remote MCP (streamable HTTP, auth, multi-tenant) is deliberately NOT in the PoC — it is a separate future stage; the transport-agnostic tool layer keeps that swap cheap.
...
- **Target environment**: Claude Cowork / Claude Desktop and ChatGPT Work on the Mac of a NON-technical user — Claude Code and Codex CLI are development surfaces, not the product. Installation is performed by the operator (VG), not the end user. Optimize tool results and skill wording for that audience.
...
- `list_messages` reports `total_messages`/`matched` and per-message attachment refs from BODYSTRUCTURE (no content download; encoded sizes; tool indices come from `read_message` — BODYSTRUCTURE order is not the flattened index space).
...
- Deferred from self-review (real, PoC-acceptable — pick up before the remote stage): `read_message` re-downloads the full RFC822 source per page (memoize raw per mailbox/uidvalidity/uid, or fetch body sections); screening is enforced per-field in handlers (consolidate into a screened view at the mail-parse boundary); the cache fingerprint is hand-maintained in `main.go` (a `Fingerprint()` on the `Screener` interface would make omissions impossible); `install.sh` duplicates the Makefile's ORT/model pins because release tarballs ship without make — keep the pins in sync.
